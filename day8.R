...
# 2페이지부터 10페이지까지 링크 클릭하여 페이지 이동하기 (2초마다이동)
for (i in 4:12) {
...
repeat{
  ...
  cat(length(reple), "\n")
  pageLink <- remDr$findElements(using='css',"#reviewSection > div:nth-child(6) > div > span:nth-child(3) > i ")#다음버튼
  remDr$executeScript("arguments[0].click();",pageLink) #클릭이벤트(자바스크립트)
  Sys.sleep(2)
  
  #다음페이지넘버와 현재페이지 넘버가 같으면 끝난다
