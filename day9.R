...
#랜더링만 해서는 댓글을 읽을수 없으므로 스크롤을 내리는 동작이 필요하다.

webElem <- remDr$findElement("css", "body")
remDr$executeScript("scrollTo(0, 0)", args = list(webElem)) #scrollTo : 절대치
Sys.sleep(1)
remDr$executeScript("scrollBy(0, 3200)", args = list(webElem)) #scrollBy : 상대치
Sys.sleep(1)
...
repeat {
  ...
  for(index in 3:7) {
    ...
    if (length(fullContentLink) == 0) { #마지막 페이지면 종료
      cat("종료\n")
      endFlag <- TRUE
      break
    }
